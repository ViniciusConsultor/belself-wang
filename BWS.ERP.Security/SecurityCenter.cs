    ...
    public class SecurityCenter
    {
        ...
        public static DataTable FormSecurityData
        {
            get
            {
                if (_dt == null && CurrentUserID != "")
                {
                    ...
                    _dt = BWS.ERP.DataAccess.DbHelperSQL.Query(sDataSql).Tables[0];
                    return _dt;
                }
                else
                {
        ...
        public static bool IsAdmin
        {
            get
            {
                if (_isadmin == false)
                {
                    object obj = BWS.ERP.DataAccess.DbHelperSQL.GetSingle("SELECT 1 FROM sysUser WHERE iUserType=1 AND sUserID='" + CurrentUserID + "'");
        ...
        public static string GetMenuAuthSQL()
        {
            ...
            {
                sResult = "SELECT A.ID,A.iFormID,A.sMenuName,A.iParentID,A.sModuleName,A.sFormClassName,A.iSort FROM sysMenu A ORDER BY A.iSort";
            }
            else
            {
                sResult = "SELECT DISTINCT E.ID,E.iFormID,E.sMenuName,E.iParentID,E.sModuleName,E.sFormClassName,E.iSort "
                        + "FROM sysRolesRights A "
                        + "LEFT JOIN sysRoles B ON A.RoleID=B.ID "
                        + "LEFT JOIN sysRolesUser C ON B.ID=C.RoleID "
                        + "LEFT JOIN sysUser D ON C.UserID=D.ID "
        ...
        public static int CheckSystemLogin(string suser,string spassword)
        {
            ...
            try
            {
                ...
                if (spassword == BWS.ERP.BaseControl.SysEncrypt.DecryptStr(spwd))
                {
                    iResult = 1;
                    if (dtTemp != null && dtTemp.Rows.Count > 0)
                    {
