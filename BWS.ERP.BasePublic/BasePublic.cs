        ...
        public static void SetAllControlsReadOnly(Control ctr, bool isreadonly)
        {
            ...
            foreach (Control ctl in ctr.Controls)
            {
                if (ctl.Tag == null || ctl.Tag.ToString() != "99")
                {
                    ...
                    else if (ctl is BWS.ERP.Controls.BWSLookUp)
                    {
                        ((BWS.ERP.Controls.BWSLookUp)ctl).IsReadOnly = !isreadonly;
                    }
                    //TextEdit
        ...
        public static void SetNotNullFiledsColor(Control ctr, List<string> fields)
        {
            foreach (string s in fields.ToArray())
            {
                foreach (Control ctl in ctr.Controls)
                {
                    if (ctl.Tag == null || ctl.Tag.ToString() != "99")
                    {
                        if (ctl is BWS.ERP.Controls.BWSLookUp && ctl.Parent is DevExpress.XtraLayout.LayoutControl)
                        {
                    ...
                    for (int i = 0; i < fields.Count; i++)
                    {
                        foreach (Control ctl in ctr.Controls)
                        {
                            if (ctl is DevExpress.XtraEditors.TextEdit)
                            {
                                ...
                                {
                                    BWS.ERP.BaseControl.Public.SystemInfo(((DevExpress.XtraLayout.LayoutControl)ctl.Parent).GetItemByControl(ctl).Text + "不能够为空！");
                                    i = fields.Count;
                                    ctl.Focus();
                                    IsNull = false;
                                    return false;
